...
from force import calculate_mov_direction_total, calculate_mov_direction
...
while np.linalg.norm(obj_point - goal) > 0.1:
    ...
    for obstacle in obstacles:
        ...
        if np.linalg.norm(position - obstacle_point) > p_obstacle:
            ...
            # #下面的代码替换掉上面的，可以得到比较平滑的轨迹，研究者可以自行尝试其他参数
            #The following code, when substituted for the above, can achieve a smoother trajectory. Researchers can experiment with other parameters.
            # if position[0] <4 :
            #     i = i + 2
            # elif position[0] <5.89:
            #     if count % 4  == 0 :
            #         i = i + 1
            # elif position[0] < 6.11:
            #     if count % 1 == 0:
            #         i = i + 2
            # else:
            #     if count % 2 == 0:
